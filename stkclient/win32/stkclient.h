...
typedef struct{
    unsigned int  uid;
    char nickname[STK_NICKNAME_SIZE];
    char city[STK_CITY_SIZE];
    unsigned int  phone;
    unsigned char gender;
    char pass[STK_PASS_SIZE];
    char serverip[STK_IPADDR_LENGTH];
    int           group_num;
    stk_group     *group;
    socket_t fd;
    int state;
