...
int stk_init_socket()
{
    ...
    {
        stk_log("stk_init_socket: init socket error\n");
        return -1;
    }
    return 0;
...
int stk_connect(client_config *config)
{
    ...
    if((config->fd = socket(AF_INET, SOCK_STREAM, 0)) < 0){
        stk_log("stk_connect: create socket error\n");
        return -1;
    }
    ...
    if( servaddr.sin_addr.s_addr == INADDR_NONE ){
        stk_log("stk_connect: inet_addr error for %s\n", config->serverip);
        return -1;
    }

    setsockopt(config->fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&sendtimeout, sizeof(sendtimeout));
    setsockopt(config->fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&recvtimeout, sizeof(recvtimeout));

    if( connect(config->fd, (struct sockaddr*)&servaddr, sizeof(servaddr)) < 0){
        stk_log("stk_connect: connect error\n");
        return -1;
    }
...
void stk_init_head(stkp_head *head, unsigned short cmd, unsigned int uid)
{
...
int stk_login(socket_t fd, char *buf, int max_len, unsigned int uid, char *password)
{
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_login: send req login msg error\n");
        return STK_CLIENT_LOGIN_ERROR;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_login: recv socket error(req login)\n");
        return STK_CLIENT_LOGIN_ERROR;
    }
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_login: send login msg error\n");
        return STK_CLIENT_LOGIN_ERROR;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_login: recv socket error(login)\n");
        return STK_CLIENT_LOGIN_ERROR;
    }
        ...
        || head->stkp_token != htonl(token))
    {
        ...
        if (*tmp == STK_LOGIN_SUCCESS) {
            ...
            stk_log("stk_login: Unknown login reply.\n");
            return STK_CLIENT_LOGIN_ERROR;
        }
    }
...
int stk_send_getbuddyinfo(socket_t fd, char *buf, int max_len, unsigned int uid, unsigned int n_uid, stk_buddy *buddy)
{
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_send_getbuddyinfo: send msg error\n");
        return -1;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_send_getbuddyinfo: recv socket error\n");
        return -1;
    }
    ...
    {
        stk_log("stk_send_getbuddyinfo: bad msg, drop packet.\n");
        return -1;
    } else {
        tmp = buf + sizeof(stkp_head);
        ...
        if (buddy->uid == 0) {
            stk_log("stk_send_getbuddyinfo: Error, bad uid.\n");
            return -1;
        }
...
int stk_send_getbuddy(socket_t fd, char *buf, int max_len, unsigned int uid)
{
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_send_getbuddy: send msg error\n");
        return -1;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_send_getbuddy: recv socket error\n");
        return -1;
    }
    ...
    {
        stk_log("stk_send_getbuddy: bad msg, drop packet.\n");
        return -1;
    } else {
        tmp = buf + sizeof(stkp_head);
        ...
        while ((i++) < buddy_num) {
            ...
            if (stk_add_buddy(&buddy) < 0) {
                stk_log("stk_send_getbuddy: Error Add buddy.\n");
                continue;
            }
        }
...
{
    stkp_head *head = NULL;
    group_member *member = NULL;
    group_member *member_tmp = NULL;
    char *tmp = NULL;
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_send_getgroupinfo: send msg error\n");
        return -1;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_send_getgroupinfo: recv socket error\n");
        return -1;
    }
    ...
    {
        stk_log("stk_send_getgroupinfo: bad msg, drop packet.\n");
        return -1;
    } else {
        tmp = buf + sizeof(stkp_head);
        ...
        if (gid_n != gid || gid_n == 0) {
            stk_log("stk_send_getgroupinfo: Error, bad gid.\n");
            return -1;
        }
...
int stk_send_getgroup(socket_t fd, char *buf, int max_len, unsigned int uid, client_config *client)
{
    ...
    stk_group *gtmp1 = NULL;
    stk_group *gtmp2 = NULL;
    int i;
    int len;
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_send_getgroup: send msg error\n");
        return -1;
    }

    len = recv(fd, buf, STK_MAX_PACKET_SIZE, 0);
    if (len == -1){
        stk_log("stk_send_getgroup: recv socket error\n");
        return -1;
    }
    ...
    {
        stk_log("stk_send_getgroup: bad msg, drop packet.\n");
        return -1;
    } else {
        tmp = buf + sizeof(stkp_head);
        ...
        while (group_num-- && gtmp1 != NULL) {
            ...
            if (stk_send_getgroupinfo(fd, buf, max_len, uid, group_uid, gtmp1) == -1) {
                stk_log("stk_send_getgroup: something wrong when get group info.\n");
            }
            gtmp1 = gtmp1->next;
        }
...
int stk_send_msg(socket_t fd, char *buf, int max_len, char *data, int data_len, 
                                unsigned int uid, unsigned int id, BOOL gmsg)
{
    char *tmp = NULL;
    unsigned int id_n;
    unsigned short cmd;
    ...
    if (len != send(fd, buf, len, 0)) {
        stk_log("stk_send_msg: send msg error\n");
        return -1;
    }
    ...
    while (1) {
        ...
        if (len == -1){
            if (WSAGetLastError() == WSAETIMEDOUT) {
                /* Recv timeout */
                //stk_log("stk_recv_msg: recv socket timeout\n");
                break;
            } else {
                stk_log("stk_recv_msg: recv socket error\n");
            ...
            case STKP_CMD_SEND_MSG:
                ...
                if (buddy == NULL)
                    ...
                    if (!stk_add_msg(buddy, data, size)) {
                        stk_msg_event(FALSE, (void *)buddy);
                    }
                }
                break;
            case STKP_CMD_SEND_GMSG:
                ...
                if (found && group != NULL) {
                    if (!stk_add_gmsg(group, data, size, uid)) {
                        stk_msg_event(TRUE, (void *)group);
                    }
                }
                break;
            default:
                stk_log("stk_recv_msg: Bad STKP CMD, Drop it.\n");
                break;
            }
        }
    }
